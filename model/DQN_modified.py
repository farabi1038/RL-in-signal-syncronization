...
def compute_value_loss(y, t, clip_delta=True, batch_accumulator='mean'):
    ...
    y = [F.reshape(i, (-1, 1)) for i in y]
    t = [F.reshape(i, (-1, 1)) for i in t]
    if clip_delta:
        loss_sum = [F.sum(F.huber_loss(i, j, delta=1.0)) for (i,j) in zip(y,t)]
        if batch_accumulator == 'mean':
            loss = [i / y[0].shape[0] for i in loss_sum]
        elif batch_accumulator == 'sum':
            loss = loss_sum
    else:
        loss_mean = [F.mean_squared_error(i, j) / 2 for (i,j) in zip(y,t)]
        if batch_accumulator == 'mean':
            loss = loss_mean
        elif batch_accumulator == 'sum':
            loss = loss_mean * y[0].shape[0]
    return loss
...
def compute_weighted_value_loss(y, t, weights,
    ...
    t = [F.reshape(i, (-1,1)) for i in t]
    if clip_delta:
        losses = [F.huber_loss(i, j, delta=1.0) for (i,j) in zip(y,t)]
    else:
        losses = [F.square(i - j) / 2 for (i,j) in zip(t,y)]
    losses = [F.reshape(i, (-1,)) for i in losses]
    loss_sum = [F.sum(i * weights) for i in losses]
    if batch_accumulator == 'mean':
        loss = [i / y[0].shape[0] for i in loss_sum]
    elif batch_accumulator == 'sum':
        loss = loss_sum
...
class DQN(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def __init__(self, q_function, optimizer, replay_buffer, gamma,
        ...
        self.average_q = [0 for _ in range(q_function.n_actions)]
        self.average_q_decay = average_q_decay
        self.average_loss = [0 for _ in range(q_function.n_actions)]
    ...
    def update(self, experiences, errors_out=None):
        ...
        # Update stats
        self.average_loss = [i * self.average_loss_decay for i in self.average_loss]
        self.average_loss = [(1 - self.average_loss_decay) * float(i.array) for i in loss]

        self.model.cleargrads()
        [i.backward() for i in loss]
    ...
    def _compute_target_values(self, exp_batch):
        ...
        next_q_max = [i.max for i in target_next_qout]
        # q = [float(i.max.array) for i in action_value]
        ...
        return [batch_rewards + discount * (1.0 - batch_terminal) * i for i in next_q_max]
    ...
    def _compute_y_and_t(self, exp_batch):
        ...
        batch_q = [F.reshape(i.evaluate_actions(
            j), (batch_size, 1)) for (i,j) in zip(qout,np.transpose(batch_actions))]

        with chainer.no_backprop_mode():
            # batch_q_target = F.reshape(
            #     self._compute_target_values(exp_batch),
            #     (batch_size, 1))
            batch_q_target = [F.reshape(i, (batch_size, 1)) for i in self._compute_target_values(exp_batch)]
    ...
    def _compute_loss(self, exp_batch, errors_out=None):
        ...
        if errors_out is not None:
            ...
            delta = [F.absolute(i-j) for i,j in zip(y,t)]
            if delta[0].ndim == 2:
                # delta = F.sum(delta, axis=1)
                delta = [F.sum(i, axis=1) for i in delta]
                temp = chainer.Variable(np.zeros(len(delta[0]), dtype=np.float32))
                for i in delta:
                    temp += i
                delta = temp / len(y)
            delta = cuda.to_cpu(delta.array)
            # delta = [cuda.to_cpu(i.array) for i in delta]
            for e in delta:
                errors_out.append(e)
        ...
        with chainer.using_config('train', False), chainer.no_backprop_mode():
            action_value =\
                self._evaluate_model_and_update_recurrent_states(
                    [obs], test=True)
            q = [float(i.max.array) for i in action_value]
            action = [cuda.to_cpu(i.greedy_actions.array)[0] for i in action_value]

        # Update stats
        self.average_q = [i*self.average_q_decay for i in self.average_q]
        self.average_q = [(i + (1 - self.average_q_decay) * j) for i,j in zip(self.average_q, q)]
        ...
        with chainer.using_config('train', False), chainer.no_backprop_mode():
            action_value =\
                self._evaluate_model_and_update_recurrent_states(
                    [obs], test=False)
            q = [float(i.max.array) for i in action_value]
            greedy_action = [cuda.to_cpu(i.greedy_actions.array)[0] for i in action_value]

        action = [self.explorer.select_action(
            idx, self.t, lambda: greedy_action, action_value=i) for (i,idx) in zip(action_value,range(len(action_value)))]
