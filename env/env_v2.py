...
from utils.functions import (clone_files, create_signals,update_signals, pack_timing_obs,
...
class foothill_v2:
    ...
    def reset(self):
        ...
        obs = np.full(self.state.reset_size, 0)
    ...
    def step_local(self, action):
        ...
        sped = 0
        self.lock = {"7216":False,"7217":False,"7218":False,"7219":False,"7503":False,"7220":False,"7221":False,"7222":False,"7223":False,"7371":False}
        if self.t == 0:
            step = 1
        else:
            step = 0
        ...
        while step < total:
            ...
            for item in loop:
                ...
                for phases in loop[item]:
                    lvl_2 = int(phases[6:7]) - 1
                    self.dict[lvl_1][lvl_2] += list(loop[item][phases].values())[0]
            mod_step = (step+(self.t*total))

            for signalid in self.all_signals.keys():
                signal_object = self.all_signals[signalid]
                t = (mod_step-1+signal_object.cycle_length-signal_object.offset)%signal_object.cycle_length

                signal_object.update(self.dict[signalid], t)

                if t==signal_object.cycle_length-1:
                    if not self.lock[signalid]:
                        self.lock[signalid] = True
                        table = update_signals(self, signalid)
                        signal_object = self.all_signals[signalid]
                        for i in range((mod_step+signal_object.cycle_length-signal_object.offset)%signal_object.cycle_length):
                            occ = [1,1,1,1,1,1,1,1]
                            signal_object.update(occ, i)
                ...
                if self.config.test.status == True or self.config.benchmark.status == True:
                    ...
                    # save timing plan
                    self.action.save_csv(save_path, used_agent, self.split, self.plan_temp, self.t)
                    ...
                    fname = f"signal_res_test_{self.t}.csv" if self.config.test.status else f"signal_res_benchmark_{self.t}.csv"
        ...
        obs = self.state.retrieve()
        if self.config.state.normalize:
            obs = obs / self.config.env.local_resolution
